...
- **Claude client is conditionally wrapped by `llm-observatory`.** `semantic_analyzer.py` builds `client` based on `OBSERVATORY_TOKEN` (`config/settings.py`): if set, it imports `MonitoredAnthropic` from `llm_observatory` (sends usage/cost metrics to `OBSERVATORY_URL`, tagged `app=codereviewx`); if unset, it falls back to a plain `anthropic.Anthropic` (dev convenience — no metrics). The wrapper proxies `messages.create` and **returns the unmodified Anthropic response**, so `message.usage` / `message.content[0].text` keep working; metric send is fire-and-forget and never blocks a review. The `from llm_observatory import …` is lazy (inside the `if`) so local dev without a token doesn't need the SDK.
- **`llm-observatory` (the SDK) requires Python ≥ 3.10.** It uses PEP 604 `X | None` syntax at module load, which raises `TypeError` on 3.9. Production is fine (Docker `python:3.12-slim`), but a local interpreter at 3.9 will crash on import **only when `OBSERVATORY_TOKEN` is set** (the lazy import). Use a 3.10+ venv locally if you want metrics, or leave the token unset.
...
`GITHUB_APP_ID`, `GITHUB_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, and one of `GITHUB_APP_PRIVATE_KEY` / `GITHUB_APP_PRIVATE_KEY_PATH`. `PORT` defaults to 8000. Optional cost knobs: `ANTHROPIC_MODEL` (defaults to `claude-haiku-4-5`) and `MAX_PATCH_CHARS` (defaults to 12000). Optional observability: `OBSERVATORY_TOKEN` (`obs_sk_...`; enables `llm-observatory` metrics) and `OBSERVATORY_URL` (defaults to the Railway instance). See `.env.example`. Supported file extensions live in `config/settings.py` (`.py`, `.js`, `.ts`, `.jsx`, `.tsx`).
